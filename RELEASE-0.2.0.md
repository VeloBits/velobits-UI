...
| `@velobitsdevs/docs`   | —     | —         | —     | no — `private: true`, and in `.changeset` `ignore` |
...
On publish this produces two tags — `@velobitsdevs/tokens@0.2.0` and
`@velobitsdevs/ui@0.2.0` — two GitHub Releases whose bodies are the `## 0.2.0` section of
...
`@velobitsdevs/ui/motion` — `PageTransition`, `Stagger`, `StaggerItem`, `FadeIn`.
...
`@velobitsdevs/tokens` adds two CSS exports: `./texture.css` and `./controls.css`. Both are
...
`@velobitsdevs/ui` widened its sibling peers from `^0.1.0` to `>=0.1.0` on
`@velobitsdevs/tokens` and `@velobitsdevs/icons`. `>=` also matches how every other peer in
...
`@velobitsdevs/icons` has an empty diff against its 0.1.0 tag — confirmed with
`git diff --stat @velobitsdevs/icons@0.1.0..origin/main -- packages/icons/`.
