...
Reference document, reconstructed from the git delta between the `@velobits-dev/*@0.1.0`
...
| ---------------------- | ----- | --------- | ----- | -------------------------------------------------- |
| `@velobits-dev/tokens` | 0.1.0 | **0.2.0** | minor | yes                                                |
| `@velobits-dev/ui`     | 0.1.0 | **0.2.0** | minor | yes                                                |
| `@velobits-dev/icons`  | 0.1.0 | 0.1.0     | none  | no — zero diff since its tag                       |
| `@velobits-dev/docs`   | —     | —         | —     | no — `private: true`, and in `.changeset` `ignore` |
...
On publish this produces two tags — `@velobits-dev/tokens@0.2.0` and
`@velobits-dev/ui@0.2.0` — two GitHub Releases whose bodies are the `## 0.2.0` section of
...
`@velobits-dev/ui/motion` — `PageTransition`, `Stagger`, `StaggerItem`, `FadeIn`.
...
`@velobits-dev/tokens` adds two CSS exports: `./texture.css` and `./controls.css`. Both are
...
`@velobits-dev/ui` widened its sibling peers from `^0.1.0` to `>=0.1.0` on
`@velobits-dev/tokens` and `@velobits-dev/icons`. `>=` also matches how every other peer in
...
`@velobits-dev/icons` has an empty diff against its 0.1.0 tag — confirmed with
`git diff --stat @velobits-dev/icons@0.1.0..origin/main -- packages/icons/`.
