...
// Deliberately NOT in the barrel. `react-hook-form` is an OPTIONAL peer and the
...
export function FormDemo() {
  ...
  return (
    <Demo
      title="Form"
      note="Subpath-only: @velobits-dev/ui/form, never the barrel. `label` and `description` are PROPS, not children — Field assembles aria-describedby before children render, so a description omitted while its id is still listed is a dangling reference, and several screen readers resolve that by announcing nothing at all, error included. Try: submit empty · then an UPPERCASE key · then the key `new-checkout` for a root error."
    >
      <Form {...form}>
        <form onSubmit={form.handleSubmit(create)} className="flex max-w-md flex-col gap-4">
          ...
          />
          {/*
           * FormError reads via useFormState, never useFormContext().formState.
           * The latter is a Proxy recording which fields the CALLER of useForm
           * subscribed to, so through context the reader is never re-rendered —
           * it paints correctly once and then never updates, which passes a
           * casual test.
           */}
          <FormError />
