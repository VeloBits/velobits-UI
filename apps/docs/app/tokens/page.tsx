...
/**
 * Every value on this page is read from `@velobits-dev/tokens` at build time.
    ...
    <div className="space-y-14">
      <section>
        ...
        <p className="max-w-3xl text-muted-foreground">
          Every value below is read from <code>@velobits-dev/tokens</code> at build time and mapped
          from the exported registries, so a token added to the package appears here without anyone
          editing this page.
        </p>
      </section>
      ...
      <Section
        ...
        intro={
          <>
            ...
            border sits above <code>--panel</code>; <code>925</code> is the dark page, dropped one
            step during the glass re-tune to widen the Tier-S perceptibility budget.
          </>
        }
      >
      ...
      <Section
        ...
        intro={
          <>
            ...
            appears in at least one contrast pair; <strong>exempt</strong> means it is deliberately
            excluded, with the reason recorded in the package and repeated below.
          </>
        }
      >
      ...
      <Section
        ...
        intro={
          <>
            ...
            <code>light</code>, <code>dark</code>, <code>darkElevated</code>) floats over arbitrary
            content and is measured against all seven worst-case backdrops. <strong>Tier S</strong>{' '}
            (<code>surfaceLight</code>, <code>surfaceDark</code>) sits on the page, so it is
            measured against exactly one thing and carries no <code>backdrop-filter</code> by
            default. Alpha floor <code>{GLASS_ALPHA_FLOOR}</code>, specular alpha{' '}
            <code>{GLASS_SPECULAR_ALPHA}</code>.
          </>
        }
      >
                ...
                {Object.entries(values).map(([prop, value]) => (
                  <div key={prop} className="flex items-center gap-2 text-sm">
                    ...
                    <dd className="flex min-w-0 items-center gap-2">
                      {typeof value === 'string' && !value.endsWith('px') && <Chip value={value} />}
      ...
      <Section
        ...
        intro={
          <>
            ...
            functions the gate uses. A pair scoped to one theme prints <code>—</code> in the other.
          </>
        }
      >
      ...
      <Section
        ...
        intro={
          <>
            A chip is never seen against the colour it is declared with. Each wash is flattened over
            the page, the panel and the Tier-S glass composite, and the text measured on the result
            — which is the gate a flat pair cannot express.
          </>
        }
      >
      ...
      <Section
        ...
        intro={
          <>
            ...
            clear <strong>{PERCEPTIBILITY_FLOOR}/255</strong> max-channel distance from what it sits
            on — Tier S from both the page and the opaque panel, since it has to read as raised
            without becoming the panel. This gate is what caught a dark overlay that composited to
